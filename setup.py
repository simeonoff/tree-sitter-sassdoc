...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_sassdoc", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_sassdoc": ["*.pyi", "py.typed"],
        "tree_sitter_sassdoc.queries": ["*.scm"],
    },
    ext_package="tree_sitter_sassdoc",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_sassdoc/binding.c",
                "src/parser.c",
            ],
            define_macros=[
